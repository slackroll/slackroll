...
"slackroll":http://github.com/rg3/slackroll/downloads is a self-contained Python script released to the public domain. You can install its package with @installpkg@ for the first time. In the future, you can use @upgradepkg@ or even @slackroll@ itself to upgrade to newer versions. Continue reading for more details.
...
You can run @slackroll help@ to get a full list of operations and a brief description of each one. A full description of each operation can be found in the "operations guide":operations-guide.html. This program is not a perfect tool. I think it can handle almost every situation and be told to do exactly what you want, but reading the change log and subscribing to the @slackware-security@ mailing list is highly recommended.

In every case you should @update@ whenever the remote tree is changed. Then, you should read the change log with the @changelog@ operation if there are new entries and, finally, the @list-transient@ operation will provide a summary of activity. There should be no transient packages after you're finished.
